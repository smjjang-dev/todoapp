...
`index.html`만 저장소 루트에 두고, 서빙 대상 정적 자산(`login.html`/`signup.html`/
`css/`/`js/`)은 모두 `static/` 아래에 모아둔다(루트는 `index.html` + 문서/설정 파일만
남도록).

- `index.html` — 메인 Todo 화면(루트). `static/css/style.css`/`static/js/app.js`를
  참조한다. 비로그인 상태면 `static/js/auth.js`의 `requireSession()`이
...
3. `smjjang-dev/todoapp` 저장소를 새로 클론, 이 폴더의 파일들을 복사해 커밋/푸시
   (`index.html`/`package.json`은 저장소 루트로, `static/`은 통째로 그대로 복사 —
   GitHubPages.md 참고)
